...
Biome (not Prettier/ESLint) is the formatter/linter of record for source files: 2-space indent, double quotes, 100-char lines. `lint-staged` runs `biome check --write` + `biome format --write` on staged JS/TS/JSON.
...
### MDX content
Posts live as `.mdx` files colocated with their route: `src/app/blog/posts/*.mdx` and `src/app/work/projects/*.mdx`. `src/utils/utils.ts` reads those directories with `fs` + `gray-matter` at build time and returns `{ metadata, slug, content }`; frontmatter keys are fixed by the `Metadata` type there (`title`, `publishedAt`, `summary`, `image`, `images`, `tag`, `team`, `link`). `next.config.mjs` sets `pageExtensions` to include `md`/`mdx`. `src/components/mdx.tsx` maps MDX elements onto Once UI components (headings get anchor links via `HeadingLink`). Adding a post = adding a file; no index to update.
...
### SEO
`Meta.generate()` in each page's `generateMetadata` and `Schema` components drive metadata; `baseURL` must be correct for these. OG images are generated at `/api/og/generate` (with `fetch`/`proxy` helpers), plus `app/sitemap.ts`, `app/robots.ts`, and `/api/rss`.
...
Remote images are restricted by `next.config.mjs` `images.remotePatterns` — currently only `www.google.com`. Add a hostname there before referencing external images.
